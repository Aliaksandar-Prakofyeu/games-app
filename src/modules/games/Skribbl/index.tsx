import {Box, Button, Heading, Stack, Text, useDisclosure, useToast,} from '@chakra-ui/react'
import {uuid} from '@utils/uuid'
import React, {useCallback, useEffect, useState} from 'react'
import {ReactSketchCanvas} from 'react-sketch-canvas'
import io from 'socket.io-client'
import {Modal} from '@components/Modal'
import {getRandomWords} from '../../../consts/game'
...
export const SkribblGameBoard: React.FC<Props> = ({username, isAdmin, roomId}) => {
        ...
        <>
            <div className="w-full flex flex-col items-center gap-5">
                ...
                <div className="w-full grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    <div
                        className="sticky top-0 flex order-5 sm:order-3 lg:order-none flex-col gap-2 col-span-3 sm:col-span-1">
                        {currentPlayers.map(({playerId, image, username}, index) => (
                            <Box
                                key={playerId}
                                className=" p-2 flex items-center justify-center gap-2 bg-gray-400 rounded-md"
                            >
